...
*/
  ...
  Highcharts.SVGRenderer.prototype.cuboid = function (shapeArgs) {
    ...
    result.front = this.path(paths[0])
      .attr({ zIndex: paths[3], 'stroke-linejoin': 'round' })
      .add(result);
    result.top = this.path(paths[1])
      .attr({ zIndex: paths[4], 'stroke-linejoin': 'round' })
      .add(result);
    result.side = this.path(paths[2])
      .attr({ zIndex: paths[5], 'stroke-linejoin': 'round' })
      .add(result);
  ...
  Highcharts.SVGRenderer.prototype.cuboidPath = function (shapeArgs) {
    ...
    var z2 =
      beta > 0
        ? (pArr[0].z + pArr[7].z + pArr[6].z + pArr[1].z) / 4
        : (pArr[3].z + pArr[2].z + pArr[5].z + pArr[4].z) / 4;
    ...
    var z3 =
      alpha > 0
        ? (pArr[1].z + pArr[2].z + pArr[5].z + pArr[6].z) / 4
        : (pArr[0].z + pArr[7].z + pArr[4].z + pArr[3].z) / 4;
...
***/
  ...
  Highcharts.wrap(Highcharts.Axis.prototype, 'render', function (proceed) {
    ...
    if (this.horiz) {
      ...
      if (!this.bottomFrame) {
        this.bottomFrame = renderer
          .cuboid(bottomShape)
          .attr({ fill: fbottom.color, zIndex: chart.yAxis[0].reversed && alpha > 0 ? 4 : -1 })
          .css({ stroke: fbottom.color })
          .add();
      } else {
      ...
      if (!this.backFrame) {
        this.backFrame = renderer
          .cuboid(backShape)
          .attr({ fill: fback.color, zIndex: -3 })
          .css({ stroke: fback.color })
          .add();
      } else {
      ...
      if (!this.sideFrame) {
        this.sideFrame = renderer
          .cuboid(sideShape)
          .attr({ fill: fside.color, zIndex: -2 })
          .css({ stroke: fside.color })
          .add();
      } else {
...
***/
  ...
  Highcharts.wrap(
    Highcharts.seriesTypes.pie.prototype.pointClass.prototype,
    'haloPath',
    function (proceed) {
    ...
    if (this.chart.is3d()) {
      ...
      Highcharts.each(this.data, function (point) {
        var c =
          ...
          point.color ||
          point.series.userOptions.borderColor ||
          point.series.color;
        point.options.borderColor = c;
        point.borderColor = c;
        point.pointAttr[''].stroke = c;
    ...
    Highcharts.each(series.data, function (point) {
      ...
      if (point.dataLabel) {
        point.dataLabel.attr({
          x:
            point.dataLabel.connX +
            -r * (1 - cos(b1)) * cos(a2) +
            (cos(a2) > 0 ? cos(b1) * d : 0) -
            point.dataLabel.width / 2,
          y:
            point.dataLabel.connY +
            -r * (1 - cos(a1)) * sin(a2) +
            (sin(a2) > 0 ? sin(a1) * d : 0) -
            point.dataLabel.height / 2,
        });
      }
    });
  });
...
***/
    ...
    if (this.chart.is3d()) {
      ...
      if (this.userOptions.tooltip) {
        this.tooltipOptions.pointFormat =
          this.userOptions.tooltip.pointFormat || default3dScatterTooltip;
      } else {
  ...
  if (Highcharts.VMLRenderer) {
    ...
    Highcharts.VMLRenderer.prototype.createElement3D =
      Highcharts.SVGRenderer.prototype.createElement3D;
