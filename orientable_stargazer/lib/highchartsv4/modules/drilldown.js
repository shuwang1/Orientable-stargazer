(function (g) {
  function s(a, b, d) {
    return (
      'rgba(' +
      [
        Math.round(a[0] + (b[0] - a[0]) * d),
        Math.round(a[1] + (b[1] - a[1]) * d),
        Math.round(a[2] + (b[2] - a[2]) * d),
        a[3] + (b[3] - a[3]) * d,
      ].join(',') +
      ')'
    );
  ...
  l.prototype.showDrillUpButton = function () {
    var a = this,
      ...
      f;
    this.drillUpButton
      ? this.drillUpButton.attr({ text: b }).align()
      : ((f = (c = d.theme) && c.states),
        (this.drillUpButton = this.renderer
          ...
          .attr({ align: d.position.align, zIndex: 9 })
          .add()
  ...
  l.prototype.drillUp = function () {
    for (
      ...
      if (((e = b[c]), e.levelNumber === d)) {
        b.pop();
        g = e.lowerSeries;
        if (!g.chart)
          for (; h--; )
            if (f[h].options.id === e.lowerSeriesOptions.id) {
              g = f[h];
              break;
            }
        g.xData = [];
  ...
  u &&
    p(u.prototype, {
      ...
      animateDrilldown: function (a) {
        ...
        if (!a)
          (i(this.points, function (a, b) {
            var i = g.Color(a.color).rgba;
            a.graphic
              .attr(g.merge(c, { start: f + b * h, end: f + (b + 1) * h }))
              [d ? 'animate' : 'attr'](
                a.shapeArgs,
                g.merge(d, {
                  step: function (a, b) {
  ...
  g.Point.prototype.doDrilldown = function (a) {
    for (
      var b = this.series.chart, d = b.options.drilldown, c = (d.series || []).length, f;
      c-- && !f;
    )
      d.series[c].id === this.drilldown && (f = d.series[c]);
  ...
  q(g.Point.prototype, 'init', function (a, b, d, c) {
    ...
    if (f.drilldown) {
      if (
        (g.addEvent(f, 'click', function () {
          f.doDrilldown();
        }),
        e)
      ) {
        if (!e._basicStyle) e._basicStyle = e.element.getAttribute('style');
        e.addClass('highcharts-drilldown-axis-label')
          .css(h.options.drilldown.activeAxisLabelStyle)
          .on('click', function () {
