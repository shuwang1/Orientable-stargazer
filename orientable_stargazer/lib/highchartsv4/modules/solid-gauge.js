...
*/
(function (a) {
  var l = a.getOptions().plotOptions,
    o = a.pInt,
    p = a.pick,
    j = a.each,
    m;
  ...
  m = {
    initDataClasses: function (b) {
      ...
      j(b.dataClasses, function (g, d) {
        ...
        c.push(g);
        if (!g.color)
          f.dataClassColor === 'category'
            ? ((i = e.options.colors), (g.color = i[k++]), k === i.length && (k = 0))
            : (g.color = h.tweenColors(
                a.Color(f.minColor),
                a.Color(f.maxColor),
                d / (b.dataClasses.length - 1),
              ));
      });
    },
    ...
    tweenColors: function (b, a, e) {
      ...
      return b.rgba.length === 0 || a.rgba.length === 0
        ? 'none'
        : (c ? 'rgba(' : 'rgb(') +
            Math.round(a.rgba[0] + (b.rgba[0] - a.rgba[0]) * (1 - e)) +
            ',' +
            Math.round(a.rgba[1] + (b.rgba[1] - a.rgba[1]) * (1 - e)) +
            ',' +
            Math.round(a.rgba[2] + (b.rgba[2] - a.rgba[2]) * (1 - e)) +
            (c ? ',' + (a.rgba[3] + (b.rgba[3] - a.rgba[3]) * (1 - e)) : '') +
            ')';
    },
  };
  a.seriesTypes.solidgauge = a.extendClass(a.seriesTypes.gauge, {
    ...
    drawPoints: function () {
      ...
      a.each(b.points, function (f) {
        var g = f.graphic,
          d = h.startAngleRad + h.translate(f.y, null, null, null, !0),
          i = (o(p(c.radius, 100)) * e[2]) / 200,
          l = (o(p(c.innerRadius, 60)) * e[2]) / 200,
          n = h.toColor(f.y, f),
          j;
        if (n !== 'none') ((j = f.color), (f.color = n));
        c.wrap === !1 && (d = Math.max(h.startAngleRad, Math.min(h.endAngleRad, d)));
        d = (d * 180) / Math.PI;
        d = { x: e[0], y: e[1], r: i, innerR: l, start: h.startAngleRad, end: d / (180 / Math.PI) };
        g
          ? ((i = d.d),
            g.attr({ fill: f.color }).animate(d, {
              step: function (b, c) {
